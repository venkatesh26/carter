      ...
      <form method="post" action="{{ route('admin.vendor.update',$info->id) }}" id="basicform">
        ...
        <div class="card-body">
          <div class="form-group">
            <label>{{ __('Name') }}</label>
            <input type="text" name="name" class="form-control" autocomplete="off" value="{{ $info->first_name }}" required="">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="paymentStatus">
             <option value="0" @if($info->paymentStatus=='0') selected @endif>{{ __('Pending') }}</option>
             <option value="1" @if($info->paymentStatus=='1') selected @endif>{{ __('Confirmed') }}</option>
           </select>
         </div>
   ...
   <div class="row">
    @if(!empty($info->info))
     <div class="col-sm-5">
      ...
      @endphp
       <p><b>{{ __('Average Delevery Time:') }} </b>{{ $info->delivery->content ?? '' }}</p>
       <!-- <p><b>{{ __('Average Pickup Time:') }} </b>{{ $info->pickup->content ?? '' }}</p> -->
       <p><b>{{ __('Support Email:') }} </b>{{ $json->email1 ?? '' }}</p>
       <!-- <p><b>{{ __('Support Email 2:') }} </b>{{ $json->email2 ?? '' }}</p> -->
       <p><b>{{ __('Support Phone :') }} </b>{{ $json->phone1 ?? '' }}</p>
