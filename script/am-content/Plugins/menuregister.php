...
function RegisterAdminMenuBar(){
	...
	if(Auth::User()->role->id == 1){
		...
		*/
		...
		$data['Packages']=array(
			...
			'active' => Request::is('shop/package*'),
			'child'=> array(
				...
				__('Pending Packages') => route('admin.shop.package.index'),
				__('Approved Packages') => route('admin.shop.package.index', array('approved' => 1)),
				__('Declined Packages') => route('admin.shop.package.index', array('approved' => 2)),
				__('Category') => route('admin.shop.category.index','type=1')
			)
		);

		if (Amcoders\Plugin\Plugin::is_active('shop')) {
			if (Auth()->user()->can('resturents.requests')) {
				$vendors[__('Restaurant Requests')] = route('admin.vendor.requests');
			}
			if (Auth()->user()->can('all.resturents')) {
				$vendors[__('All Restaurants')] = route('admin.vendor.index');
			}
	...
	if(Auth::User()->role->id == 3 || Auth::User()->role->id == 5){
		...
		if(Auth::User()->paymentStatus == 1){
			if (Amcoders\Plugin\Plugin::is_active('shop')) {
				...
				// 		// __('Manage Products') => route('store.product.index'),
				// 		// __('Addon Products') => route('store.addon-product.index'),
				// 		__('Packages') => route('store.package.index')
				// 	)
				// );
				...
				$data['Packages']=array(
					...
					'child'=> array(
						...
						__('List') => route('store.package.index'),
						__('Category') => route('store.menu.index'),
						__('Addon Products') => route('store.addon-product.index'),
						// __('AddOn') => route('store.addon.index')
					)
				);
