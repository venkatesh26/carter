...
$(document).on("click","#step1_submit",function() {
    // alert();
    if($("#step1_form").valid()){
        var form_1_data = new FormData(document.getElementById('step1_form'));
        console.log(form_1_data)
        $("#step1_div").removeClass('d-block');
        $("#step1_div").addClass('d-none'); 

        $("#step2_div").removeClass('d-none');
        $("#step2_div").addClass('d-block'); 
    }
});
$(document).on("click","#step2_submit",function() {
    if($("#step2_form").valid()){
        $("#step2_div").removeClass('d-block');
        $("#step2_div").addClass('d-none'); 

        $("#step3_div").removeClass('d-none');
        $("#step3_div").addClass('d-block'); 
    }
});

$(document).on("click","#step3_submit",function() {
    alert('final submit');
    if($("#step3_form").valid()){
        var form_1_data = new FormData($('#step1_form')[0]);
        var form_2_data = new FormData($('#step2_form')[0]);
        var form_3_data = new FormData($('#step3_form')[0]);
        form_3_data.append(form_1_data);
        // $("#step1_form").submit();
        // $("#step2_form").submit();
        $("#step3_form").submit();
    }
