...
class StoreController extends controller
{
	...
	public function show($slug)
	{
        ...
        $packages = $package->select('id', 'name', 'description', 'price')->where('user_id',$user_id)->get()->toArray();
        $packageItems = [];
        if (count($packages) > 0) {
            $packageIds = array_column($packages, "id");
            $packageItems = $packageItem->select('id', 'package_id', 'name', 'description', 'spicy')->whereIn('package_id', $packageIds)->where('deleted', 0)->get()->toArray();
        }
