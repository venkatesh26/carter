...
class OrderController extends controller
{
	...
	public function details($id)
	{
		...
		$riders = $riders ?? [];
		return view('theme::admin.order.details',compact('info','riders', 'orderdetails'));
	}
    ...
    $customerInfo = json_decode($order->data);
		...
		$SmsContent = "Hello ".$customerInfo->name." Your order has been ";
			if ($request->status == 3) {
			$SmsContent .= 'accepted.';
	      }
	      elseif($request->status == 1){
	      	$SmsContent .= 'completed.';
	      }
	      else{
	      	$SmsContent .= 'cancelled.';
	      }

	      // 447899357632
		$message = $twilio->messages
          ->create("+44".$customerInfo->phone, // to
                   array(
