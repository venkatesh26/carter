...
class StoreController extends controller
{
	...
	public function show($slug)
	{
        ...
        $packages = $package->select('id', 'name', 'description', 'price', 'halal_status')->where('user_id',$user_id)->get()->toArray();
        $packageItemsLIst = [];
		$packageItems = array();
        if (count($packages) > 0) {
			foreach($packages as $package_row){
				...
				$packageItemsLIst = $packageItem->select('package_items.id', 'package_items.package_id', 'package_items.name', 'package_items.description', 'package_items.spicy','package_items.mild','package_items.hot','package_items.extra_hot','package_items.none','package_items.package_category_id','categories.name as category_name','package_categories.category_id','packages.name as package_name')
  ...
  //       JsonLd::addImage($store->preview->content ?? $store->avatar);
		...
		if($store && $store->role_id == 3 && $store->status == 'approved' || $store->status == 'offline') {
			...
			return view('theme::store.index',compact('store','five_rattings','four_rattings','three_rattings','two_rattings','one_rattings','packages','auth_id', 'packageItems'));
		}else{
			return abort(404);
		}
	}
	...
	public function showpakages($slug)
	{
  ...
  //       JsonLd::addImage($store->preview->content ?? $store->avatar);
		...
		if($store && $store->role_id == 3 && $store->status == 'approved' || $store->status == 'offline') {
			// $galleries = explode(",", $store->gallery->content);
			// return view('theme::store.index',compact('store','galleries','five_rattings','four_rattings','three_rattings','two_rattings','one_rattings'));
			return view('theme::store.showpakage',compact('store','five_rattings','four_rattings','three_rattings','two_rattings','one_rattings','packages','auth_id', 'packageItems'));
		}else{
			return abort(404);
		}
	}
