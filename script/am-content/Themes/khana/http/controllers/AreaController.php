...
class AreaController extends controller
{
	...
	public function show(Request $request,$id)
	{
			...
			// echo$delivery = $request->delivery['del'][0];

		// print_r($request->cats["cats"]);

		if(isset($request->cats) && isset($request->cats["cats"])){
			...
			// User::->where('user_category.category_id',$request->cats)
		...
		if ($id==0) {
			...
			$posts = User::with('info','gallery','preview','avg_ratting','delivery','pickup','shopcategory','location','shopday','role','vendor_reviews','ratting','badge','usersaas')->where('status', "approved")->where('status', "approved")->where('role_id', 3)->paginate(150);
				...
				if(!empty($postcode1)){
					...
					if($result["status"] == "OK"){
					    if(count($result['rows'])>0){
					    	if($result['rows'][0]['elements'][0]["status"] !="ZERO_RESULTS" && $result['rows'][0]['elements'][0]["status"] !="NOT_FOUND"){
