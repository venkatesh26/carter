...
$(document).ready(function () {
    ...
    $(document).on("click", ".add-more", function () {
        ...
        let copyEl = '<div class="row"><div class="col-md-4"><div class="form-group mb-0"><label>Item Name</label><input type="text" class="form-control item-name ui-autocomplete-input" name="item[' + thiCatId + '][name][]" data-parsley-required="" autocomplete="off"></div></div><div class="col-md-4"><div class="form-group mb-0"><label>Item Description</label><input type="text" class="form-control item-description" name="item[' + thiCatId + '][description][]" data-parsley-required=""></div></div><div class="col-md-3"><div class="form-group mb-0"><label for="uname">Spicy</label><br><div class="form-check-inline"><label class="form-check-label"><input type="radio" class="form-check-input item-spicy spicy-mild" name="item[' + thiCatId + '][spicy][]" value="mild" checked="" data-parsley-multiple="itemspicy">Mild</label></div><div class="form-check-inline"><label class="form-check-label"><input type="radio" class="form-check-input item-spicy spicy-hot" name="item[' + thiCatId + '][spicy][]" value="hot" data-parsley-multiple="itemspicy">Hot</label></div><div class="form-check-inline"><label class="form-check-label"><input type="radio" class="form-check-input item-spicy spicy-extra_hot" name="item[' + thiCatId + '][spicy][]" value="extra_hot">Extra Hot</label></div><div class="form-check-inline"><label class="form-check-label"><input type="radio" class="form-check-input item-spicy spicy-none" name="item[' + thiCatId + '][spicy][]" value="none">None</label></div></div></div><div class="col-md-1 remove-item"><label for="uname">&nbsp;</label><br><p class="btn btn-danger  text-right mt-0 mb-3 remove-icon" style="color: #fff;font-size: inherit;" data-catid="'+thiCatId+'">Remove</p></div></div>';
    ...
    $(document).on("click", ".remove-cat-icon", function () {
        let thiCatId = $(this).data("catid");
        let getPrdtCat = $(this).closest(".checkoldproctcat");

        if(confirm("Are you sure want to remove this?")){
            if(getPrdtCat.length>0){
                let getPrdtCatId = getPrdtCat.data("prdtcatid");
                 deletePrdtCat(getPrdtCatId);
...
function deletePrdtCat(post_id){

    $.ajax({
        type: 'post',
        url: '/store/package/delete/'+post_id,  
